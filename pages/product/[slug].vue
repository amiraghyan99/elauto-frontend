	...
	meta: [
		{
			hid: 'description',
			name: 'description',
			content: product?.rawDescription ?? ''
		}
	]
});
...
const disabledAddToCart = computed(
	() => (!activeVariation.value && !!product.variations) || type.value.stockStatus !== 'IN_STOCK'
);
				...
				<div class="flex justify-between mb-4">
					<div class="flex-1">
						<h1 class="flex flex-wrap items-center gap-2 mb-2 text-2xl font-sesmibold">
							{{ type.name }}
							<WPAdminLink :link="`/wp-admin/post.php?post=${product.databaseId}&action=edit`">
								Edit
							</WPAdminLink>
						</h1>
