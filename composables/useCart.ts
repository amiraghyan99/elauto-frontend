...
export function useCart() {
	...
	const isShowingCart = useState<boolean>('isShowingCart', (): boolean => false);
	const isUpdatingCart = useState<boolean>('isUpdatingCart', (): boolean => false);
	const isUpdatingCoupon = useState<boolean>('isUpdatingCoupon', (): boolean => false);
	const paymentGateways = useState<PaymentGateway[]>('paymentGateways', (): PaymentGateway[] => []);
	...
	// Apply coupon
	async function applyCoupon(code: string): Promise<{
		message: string | null;
	}> {
		try {
			isUpdatingCoupon.value = true;
			const { applyCoupon } = await GqlApplyCoupon({
				code
			});
	...
	async function removeCoupon(code: string): Promise<void> {
		try {
			const { removeCoupons } = await GqlRemoveCoupons({
				codes: [code]
			});
