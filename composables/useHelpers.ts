...
export function useHelpers() {
	...
	const wooNuxtVersionInfo: string = config.public.version || '0.0.0';
	const productsPerPage: number = config.public.productsPerPage || 24;
	...
	function replaceQueryParam(param: string, value: string, search: string): string {
		...
		for (let index = 0; index < numberOfVariation; index++) {
			...
			product.variations?.nodes.forEach(element => {
				if (element.attributes?.nodes[index]?.value)
					tempArray.push(element.attributes.nodes[index].value as string);
			});
