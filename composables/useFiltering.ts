	...
	function filterProducts(products: Product[]): Product[] {
		return products.filter(product => {
			...
			const categoryCondition = category.length
				? product.productCategories?.nodes?.find((node: any) => category.includes(node.slug))
				: true;
			...
			const productPrice = product.rawPrice
				? parseFloat([...product.rawPrice.split(',')].reduce((a, b) => String(Math.max(Number(a), Number(b)))))
				: 0;
			...
			const ratingCondition = starRating.length
				? product.averageRating
					? product.averageRating >= parseFloat(starRating[0])
					: false
				: true;
			...
			const attributeCondition = globalProductAttributes
				.map((attribute: string) => {
					...
					return product.terms?.nodes?.find(
						(node: any) => node.taxonomyName === attribute && attributeValues.includes(node.slug)
					);
				})
			...
			return (
				ratingCondition && priceCondition && attributeCondition && categoryCondition && saleItemsOnlyCondition
			);
		});
	}
