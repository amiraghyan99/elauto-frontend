...
const productCategoryPermallink = runtimeConfig?.public?.PRODUCT_CATEGORY_PERMALINK || '/car-category/';
...
<template>
	<div class="flex text-sm leading-none text-gray-400 gap-1 items-center">
		...
		<span v-for="(link, i) in format" :key="link.name || i">
			<NuxtLink v-if="link.slug" :to="formatURI(link.slug)" class="hover:text-primary">
				{{ link.name }}
			</NuxtLink>
